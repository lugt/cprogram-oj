...
public:
  ...
  BiTreeNodeT * Add_node_as_left(UINT val, BiTreeNodeT *parent){
    if(parent) {
...
INT main() {
  ...
  for(UINT i = 0; i < trials; i++) {
    ...
    for(;(ch = (CHAR) getchar()) && ch != '\n' && ch != EOF && ch != '\r';){
      ...
      if(isalpha(ch)) {
        ...
        if(cur_parents.size() == 0){
          BiTreeNodeT * node = tree_1.Add_node_as_left(ch, NULL);
          root = node;
          ...
          for(;;){
            if(top->can_add_left()){
              cur_parents.push(tree_1.Add_node_as_left(ch, top));
              break;
            }else if(top->can_add_right()){
              cur_parents.push(tree_1.Add_node_as_right(ch, top));
              break;
            }else{
              if(top->_parent) {
                top = top->_parent;
              }else{
                Is_True(FALSE, ("[Error] cannot add to tree %c", ch));
                break;
              }
            }
          }
        ...
        Is_Print(Printing(), (" -- Adding 0  \n"));
        if(cur_parents.size() == 0) {
          Is_True(FALSE, ("[Error] parent .size() == 0 "));
        }
        UINT resolve = 0;
        BiTreeNodeT * top = cur_parents.top();
        while(!resolve) {
          if (top->can_add_left()) {
            tree_1.Add_node_as_left(0, top);
            break;
          } else if (top->can_add_right()) {
            tree_1.Add_node_as_right(0, top);
            break;
          } else {
            if(top->_parent) {
              top = top->_parent;
            }else{
              Is_True(FALSE, ("[Error] cannot add to tree :: 0"));
            }
          }
        }
      }else{
    ...
    if(Printing()) {
      Is_Print(Printing(), ("%s", (Print_all(), "-------- END --------")));
    }
    if(parsed > 0) {
      leav_count = 0;
