...
void l_App(CONNECTOR * &pCONNECTOR, INT rhs) {
  CONNECTOR * cursor = pCONNECTOR;
  if(cursor != NULLPTR){
    while (TRUE){
    ...
    pCONNECTOR = cursor = new CN();
    pCONNECTOR->next = NULLPTR;
    pCONNECTOR->pos = rhs;
  }
}
...
void one_trial(){
  ...
  for(UINT i = 0; i < edge_count; i++){
    ...
    l_App(vl[lhs]->first, rhs);
  }

  for(UINT i = 0; i < vex_count ; i++){
    cout << i << " " << all_nodes[i];
    CN *cursor = vl[i]->first;
